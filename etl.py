    ...
    with conn.begin() as conn:
       response= conn.execute(text("""
            ...
            when not matched then
	            insert(unique_key,coord, weather, main, dt, base, name, visibility, wind, clouds, sys, timezone, rain, cod,id, _record_loaded_at, inserted)
	            values(s.unique_key, s.coord, s.weather, s.main, s.dt, s.base, s.name, s.visibility, s.wind, s.clouds, s.sys, s.timezone, s.rain, s.cod, s.id, now(),"inserted")
    """))
